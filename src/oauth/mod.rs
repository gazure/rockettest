...
async fn token_endpoint(token_request: TokenRequestForm<'_>, clients: Clients<'_>) -> Result<Value, Status> {
	let token = server::token(token_request, clients).await.map_err(|e| -> Status {e.into()})?;
	Ok(json!(token))
}
...
#[get("/clients/<id>")]
async fn get_client(id: Uuid, clients: Clients<'_>, auth: client_jwt::ClientJwt) -> Result<Json<Client>, Status> {
	auth.authorize_for(&id).map_err(|e| -> Status {e.into()})?;
	clients.get(&id).await.map_or(Err(Status::NotFound), |client| Ok(Json(client)))
}


#[delete("/clients/<id>")]
async fn delete_client(id: Uuid, clients: Clients<'_>, auth: client_jwt::ClientJwt) -> Result<NoContent, Status> {
	auth.authorize_for(&id).map_err(|e| -> Status {e.into()})?;
...
pub async fn stage() -> rocket::fairing::AdHoc {
	...
	rocket::fairing::AdHoc::on_ignite("oauth", |rocket| async {
		rocket.mount("/oauth", routes![token_endpoint, register, get_client, delete_client])
...
mod test {
    ...
    async fn test_register_client() {
        ...
        let delete_response = test_client.delete(format!("/oauth/clients/{}", client.id))
            .header(Header::new("Authorization", format!("Bearer {}", token.access_token)))
            .dispatch().await;
