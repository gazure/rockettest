...
pub async fn generate(grant_type: GrantType, scopes: Vec<Scope>, client: Client) -> Result<Token, Error> {
    match grant_type {
        GrantType::ClientCredentials => {
            ...
            let scopes_string = scopes.iter().map(|s| s.to_string()).collect::<Vec<String>>().join(" ");
...
mod test {
    ...
    async fn test_generate_client_credentials() {
        ...
        let token = generate(GrantType::ClientCredentials, scopes, client).await.unwrap();
